...
(() => {
  ...
  const SEED_KEY = "dj_seed_agenda_v1";
  const SEED_SHOWS = [];

  /* Curadores iniciais, se houver. Cada um entra como contato e já abre
  ...
  /* Aeroportos usados nas viagens — só pra mostrar a
  ...
  /* Passagens já compradas, se houver. Mesmo esquema dos outros lotes:
     entra uma vez por aparelho, com id fixo pra não duplicar. */
  const SEED_LOGISTICA_LOTES = [];
  const SEED_LOGISTICA_KEY = "dj_seed_logistica_v1";
  ...
  let contactFilter = "";
  const GRUPOS_KEY = "dj_grupos_abertos_v1";
  ...
  function prettyPhone(raw) {
    const str = (raw || "").trim();
    // fora do Brasil, mantém como foi escrito — o formato (DD) é daqui
  ...
  async function connectFirebase() {
    ...
    try {
      ...
      dealsCol = fsMod.collection(db, "dj_negociacoes");
      contactsCol = fsMod.collection(db, "dj_contatos");
      logCol = fsMod.collection(db, "dj_logistica");
      configDocRef = doc(db, "dj_config", "app");
  ...
  /* Mesma ideia dos complementos de contato, pra logística: preenche
     campo vazio de um voo que já entrou, sem tocar no que já foi editado à mão. */
  const COMPLEMENTOS_LOG_KEY = "dj_complementos_log_v1";
  ...
  /* ── Estimativa de época cheia ──────────────────────────
     ...
     1. Temporada, e ela depende do destino. Fevereiro em Zurique é
        ...
        que interessa enxergar — por isso cada região tem seu perfil.
  ...
  /* Busca por cidade, país ou sigla. O datalist do HTML não servia: o
     ...
     pelo value — ou seja, pela sigla, que é justamente o que quem digita não
  ...
  /* ── Cidade a partir do telefone ────────────────────────
     ...
     isso o melhor que dá é o país, até digitarem a cidade. */
  ...
  /* Contatos a remover, se houver. Roda uma vez por aparelho.
  ...
  const REMOCOES_KEY = "dj_remocoes_v1";
  const REMOCOES = [];
  ...
  function buildMessage(templateKey, deal) {
    ...
    return tpl
      ...
      .replaceAll("{presskit}", presskitUrl())
      .replaceAll("{presskitEn}", presskitEnUrl())
  ...
  /* Avisa antes de mandar uma mensagem que depende de um link que
     ainda não foi cadastrado — evita enviar um texto capenga ao curador. */
  ...
  document.querySelectorAll("[data-presskit]").forEach((btn) =>
    btn.addEventListener("click", () => {
      const url = btn.dataset.presskit === "en" ? presskitEnUrl() : presskitUrl();
      if (!url) return showToast("Cadastre o link do seu press kit em Configurações.");
  ...
  document.querySelectorAll("[data-copy-presskit]").forEach((btn) =>
    btn.addEventListener("click", async () => {
      const url = btn.dataset.copyPresskit === "en" ? presskitEnUrl() : presskitUrl();
      if (!url) return showToast("Cadastre o link do seu press kit em Configurações.");
      try {
  ...
  document.querySelectorAll("[data-c-presskit]").forEach((btn) =>
    btn.addEventListener("click", () => {
      const url = btn.dataset.cPresskit === "en" ? presskitEnUrl() : presskitUrl();
      if (!url) return showToast("Cadastre o link do seu press kit em Configurações.");
  ...
  /* Vira lista de shows, já descartando o que não interessa.
     ...
     conferir se exportou da conta certa. */
  ...
  /* ── Google Agenda pela API (opcional) ───────────────────
     ...
     escrever nem apagar nada na sua conta. O token vale ~1h e
  ...
  document.getElementById("export-csv").addEventListener("click", () => {
    ...
    downloadCsv("dj-negociacoes.csv", dealRows);
    setTimeout(() => downloadCsv("dj-contatos.csv", contactRows), 400);
  ...
  async function unificarCacheOnce() {
    const CHAVE = "dj_cache_unificado_v1";
    try {
  ...
  /* Habilita instalar como app no Mac e no iPhone. O service worker é
     ...
     antiga — mas o ?v= do dj.html continua valendo. */
  if ("serviceWorker" in navigator) {
    window.addEventListener("load", () => {
      navigator.serviceWorker.register("sw-dj.js").catch(() => {});
    });
  }
})();
